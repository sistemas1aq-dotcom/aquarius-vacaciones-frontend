/**
 * Reportes Analíticos — la foto de cómo va el año: gozadas por mes, resumen
 * por departamento y el top de saldos pendientes.
 *
 * Nota sobre el año: el gráfico mensual y la columna «Gozados 2026» están
 * fijados a 2026 tal como estaban en la versión anterior. Cuando cambie el
 * ejercicio habrá que parametrizarlo, igual que el selector de año del
 * reporte corporativo.
 */
...
export class ReportesAnaliticosComponent implements OnInit {
  ...
  ngOnInit() {
    // Los 12 meses siempre presentes: un mes sin vacaciones es un dato, no
    ...
    this.api.getMonthlyReport(2026).subscribe(data => {
      data.forEach(d => {
        const idx = d.Month - 1;
        if (idx >= 0 && idx < 12) this.monthlyData[idx] = d;
      });
